...
void ShowHelpConsole(void)
{
  ...
  "[m]                       reconnect to the CEC adapter." << std::endl <<
  "[?] or [help]             show this help." << std::endl <<
  "[CTRL - C] or [quit]             to quit the CEC test client and switch off all" << std::endl <<
  "                          connected CEC devices." << std::endl <<
  "================================================================================" << std::endl;
}


bool aWholeNewWorld(ICECAdapter *parser, const std::string &command, std::string & UNUSED(arguments)){
...
if(!jordanstart){
      ...
      "(to show this list of device numbers again press \"I\")\n";
      "(to change input without showing the list press \"i\")\n";
      std::cout << "Which Device Number (or lowercase letter) would you like to control?:";
      std::cin.sync(); //clear buffer
      std::cin.get(); //wait
      deviceNum = '\0';//clears device num so the loop can be ran
      while(!std::isalnum(deviceNum)){
        deviceNum = getchar();
      }
      std::cout << "you have chosen:" << deviceNum << "\n\n";
...
// bool setDeviceNumber(ICECAdapter *parser, const std::string &command, std::string & UNUSED(arguments)){
// // if (setDevice){
// //   deviceNum = command;
// //   setDevice = false;
// // }
...
bool ProcessCommandPeePeePooPoo(ICECAdapter *parser, const std::string &command, std::string &arguments)
{

  std::string strvalue;

  byteCommand = "1";
  byteCommand += deviceNum;


      if(command == RIGHT) byteCommand += ":44:04";
      else if(command == LEFT) byteCommand += ":44:03";
      else if(command == DOWN) byteCommand += ":44:02";
      else if(command == UP) byteCommand += ":44:01";
      else if(command == SELECT) byteCommand += ":44:00";
      else if(command == EXIT) byteCommand += ":44:0d";
      else if(command == ROOTMENU) byteCommand += ":44:09";
      else if(command == SETUPMENU) byteCommand += ":44:0A";
      else if(command == EJECT) byteCommand += ":44:4A";
      else if(command == PLAY) byteCommand += ":44:44";
      else if(command == REWIND) byteCommand += ":44:48";
      else if(command == FASTFORWARD) byteCommand += ":44:49";
      else if(command == STOP) byteCommand += ":44:45";
      else if(command == RECORD) byteCommand += ":44:47";
      else if(command == STANDBY) byteCommand += ":36";
      else if(command == CLEAR) byteCommand += ":44:2c";
      else if(command == LISTDEVICE) jordanstart = false;
      else if (command == POWERON)
        {
          std::string s;
          s += deviceNum;
          std::string &arg = s;//had to do the other steps because of pointer stuff rules I essentially treat "deviceNum" to be the argument in the default power comand. deviceNum is the number chosen by he user
          std::string strValue;
          uint8_t iValue = 0;
          if (GetWord(arg, strValue) && HexStrToInt(strValue, iValue) && iValue <= 0xF)
          {
            parser->PowerOnDevices((cec_logical_address) iValue);
            return true;
          }
      ...
      else return false;//this is necessary fot the case of no input


  std::string &filler = byteCommand;



  cec_command bytes = parser->CommandFromString(filler.c_str());

  bytes.transmit_timeout = 100;
...
bool ProcessCommandJORDANChangeDevice(ICECAdapter *parser, const std::string &command, std::string &arguments){


  if (command == CHANGEDEVICE){
  ...
  std::cout << "(to show the list of device numbers press \"I\")\n";
  std::cout << "Which Device Number (or lowercase letter) would you like to control?:";
  std::cin.sync(); //clear buffer
  std::cin.get(); //wait
  deviceNum = '\0';
...
bool ProcessConsoleCommand(ICECAdapter *parser, std::string &input)
{
  if (!input.empty())
  {
    ...
    if (GetWord(input, command))
    {
      ...
      aWholeNewWorld(parser, command, input) ||
      //setDeviceNumber(parser, command, input) ||
      ProcessCommandTX(parser, command, input) ||
      //ProcessCommandTX2(parser, command, input) ||
      ProcessCommandON(parser, command, input) ||
      ProcessCommandSTANDBY(parser, command, input) ||
      ProcessCommandPeePeePooPoo(parser, command, input) ||
      ProcessCommandJORDANChangeDevice(parser, command, input) ||
...
int main (int argc, char *argv[])
{
  ...
  const char* jordanInputPointer = &jordanInput;

  std::cout << "Welcom to Jordan's Mod of cec-client!!!\n";
...
/////////////////////////I also added this/////////////////////////I also added this /////////////////////////I also added this /////////////////////////I also added this /////////////////////////I also added this
  ...
  while (!g_bExit && !g_bHardExit)
  {
    std::string input;
#if defined(HAVE_CURSES_API)
    if (!g_cursesEnable) {////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////I JORDAN MURTI EDITED HEREEEEE
      //std::cout << "Oh hi Mark";
      // getline(std::cin, input);
      // std::cin.clear();
      //std::cout << "How is your sex life";
      ...
      buffer = getchar();
      buffer2 = getchar();
      input.assign(1,jordanInput);//this helps you hold down a key with no glich
      system ("stty echo"); // Make echo work
      system("stty -cbreak");// go to COOKED mode


////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////I JORDAN MURTI EDITED Above HEREEEEE
...
#else
    //std::cout << "heeeeyy chipp chip chip";
    getline(std::cin, input);
    std::cin.clear();
    //std::cout << "hahahahaha";
#endif
